...
def loginuser(request):
    if request.method == 'GET':
        ...
        if user is None:
            return render(request, 'todo/loginuser.html', {'form': AuthenticationForm(), 'error': 'wrong username or password'})
        else:
            login(request, user)
            return redirect('currenttodo')
...
def completedtodo(request, todo_pk):
    ...
    if request.method == 'POST':
        todo.datecomepleted = timezone.now()
        todo.save()
        return redirect('currenttodo')
...
def viewtodo(request, todo_pk):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            return render(request, 'todo/viewtodo.html', {'form': form, 'error': 'bad data passed in'})
